...
impl SlskdClient {
    async fn new(base: &str, user: &str, pass: &str) -> Result<Self, String> {
        ...
        let resp: Value = client
            ...
            .send()
            .await
            .map_err(|e| format!("slskd login: {e}"))?
            .json()
            .await
            .map_err(|e| format!("slskd login parse: {e}"))?;
        ...
        for attempt in 0..timeout {
            ...
            for dir in &dirs {
                ...
                for f in &dir_files {
                    ...
                    if !filenames
                        .iter()
                        .any(|n| fname.ends_with(n.as_str()) || n.ends_with(fname))
                    {
                        continue;
                    }
                    let state = f.get("state").and_then(Value::as_str).unwrap_or("");
            ...
            if attempt % 4 == 0 {
                println!(
                    "    waiting... completed={} transferring={} pending={} failed={}",
                    completed.len(),
                    transferring,
                    pending,
                    failed
            ...
            if !any_started && attempt >= stall_limit {
                return Err(format!(
                    "peer {username} not responding after {}s, skipping",
                    stall_limit * 5
                ));
            }
        }
...
fn score_candidates(results: &Value, artist: &str, album: &str) -> Vec<AlbumCandidate> {
    ...
    for response in responses {
        ...
        let upload_speed = response
            ...
            .unwrap_or(0);
        let free_upload_slots = response
            .get("freeUploadSlots")
            .and_then(Value::as_u64)
            .unwrap_or(0);
        ...
        let mut dirs: std::collections::HashMap<String, Vec<(String, bool)>> =
...
async fn process_album(
    ...
    if dry_run {
        ...
        println!(
            "    slskd: {} files from {} (score {})",
            best.flac_count, best.username, best.score
        );
        println!("    folder: {}", best.folder);
        println!(
            "    [dry-run] would download {} files from {}",
            best.files.len(),
            best.username
        );
        return Ok(0);
    ...
    for (ci, best) in candidates.iter().enumerate().take(5) {
        println!(
            "    slskd: {} files from {} (score {}) [candidate {}/{}]",
            best.flac_count,
            best.username,
            best.score,
            ci + 1,
        ...
        // Queue download
        slskd
            .download_folder(&best.username, &best.folder, &best.files)
            .await
        ...
        match slskd.wait_for_downloads(&best.username, &best.files).await {
            Ok(completed_remote) if !completed_remote.is_empty() => {
                ...
                let mut filenames: Vec<String> = completed_remote
                    .iter()
            ...
            Ok(_) => {
                ...
                println!(
                    "    -> peer {} delivered nothing, trying next candidate...",
                    best.username
                );
            }
            Err(e) => {
                last_err = e.clone();
                println!(
                    "    -> peer {} failed: {e}, trying next candidate...",
                    best.username
                );
            }
        }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let slskd_url =
        read_setting(&db, "slskd_url").unwrap_or_else(|| "http://localhost:5030".to_string());
    ...
    let slskd_downloads_dir = read_setting(&db, "slskd_downloads_dir")
        .unwrap_or_else(|| "A:\\Staging\\slskd".to_string());
    let library_base = read_setting(&db, "library_base").unwrap_or_else(|| "A:\\Music".to_string());

    println!("Connecting to slskd at {slskd_url}...");
    let slskd = SlskdClient::new(&slskd_url, &slskd_user, &slskd_pass)
        .await
    ...
    let jobs: Vec<AlbumJob> = if let Some((artist, album)) = single_album {
        ...
        missing
            ...
            .filter(|a| {
                ...
                !lib.join(sanitize_component(&artist_s))

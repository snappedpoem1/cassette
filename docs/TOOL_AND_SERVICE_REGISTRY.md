...
| Discogs | External metadata API | Metadata search/discography fallback plus enricher release context | Label, format, country, catalog no., secondary release enrich | API/auth and rate limits; credentials not configured at 2026-04-06 proof run | Secondary enrich, not canonical identity | Client implemented; credentials not configured at proof time |
| Last.fm | External metadata API | Player context info, duration enrichment, and recent-track history sync | Tags, bios, related-artists context, user-facing enrichment | Not canonical release authority; credentials not configured at 2026-04-06 proof run | Contextual enrich for artist/album display and history sync | Client implemented; credentials not configured at proof time |
